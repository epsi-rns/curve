// ============================================================
// Bezier Quartic 3D -- Interactive UI + Curvature Comb
// Homework 2: P0(0,0,1) P1(0,4,1) P2(4,0,1) P3(4,4,1) P4(5,4,1)
// ============================================================
funcprot(0);

// -- 1. Global Data ------------------------------------------
global P u h_slider h_pts h_axes h_drawn h_comb_check;
...
// Basis matrix M for quartic Bezier (degree 4)
function M = bezier_M()
    M = [1  -4   6  -4  1; ..
        -4  12 -12   4  0; ..
         6 -12   6   0  0; ..
...
// First derivative of P(u) = U·M·P
// U'  = [4u³, 3u², 2u, 1, 0]
function d1 = bezier_d1(u_val, Pts)
    M   = bezier_M();
    Up  = [4*u_val^3, 3*u_val^2, 2*u_val, 1, 0];
    d1  = Up * M * Pts;   // row vector (1×3)
endfunction

// Second derivative of P(u)
// U'' = [12u², 6u, 2, 0, 0]
function d2 = bezier_d2(u_val, Pts)
    M   = bezier_M();
    Upp = [12*u_val^2, 6*u_val, 2, 0, 0];
    d2  = Upp * M * Pts;  // row vector (1×3)
endfunction

// Curvature kappa at a single u — 3D formula
// kappa = |P' x P''| / |P'|^3
function kap = bezier_kappa(u_val, Pts)
    d1  = bezier_d1(u_val, Pts);
    d2  = bezier_d2(u_val, Pts);
    // 3D cross product d1 × d2
    cx  = d1(2)*d2(3) - d1(3)*d2(2);
    cy  = d1(3)*d2(1) - d1(1)*d2(3);
    cz  = d1(1)*d2(2) - d1(2)*d2(1);
    cross_norm = sqrt(cx^2 + cy^2 + cz^2);
    speed      = norm(d1);
    if speed > 1e-10 then
        kap = cross_norm / speed^3;
    else
        kap = 0;
    end
endfunction

// Curvature comb — spikes perpendicular to curve via principal normal
// Principal normal: N = (P'×P'') × P' / |(P'×P'')×P'|
function draw_comb(Pts)
    N_PTS   = 120;
    N_SPIKES = 60;
    step    = max(1, floor(N_PTS / N_SPIKES));

    // Basis matrix and dense u values for auto-scale
    M    = bezier_M();
    tv   = linspace(0, 1, N_PTS)';
    Umat = [tv.^4, tv.^3, tv.^2, tv, ones(N_PTS,1)];
    cpts = Umat * M * Pts;   // (N_PTS × 3) curve points

    // Curve extent for auto-scale
    ext = max(max(cpts) - min(cpts));
    if ext < 1e-10 then ext = 1; end

    // Collect kappas at spike positions for 95th-percentile scale
    kappas = [];
    idx    = 1;
    for i = 1:N_PTS
        if modulo(i-1, step) == 0 then
            kappas(idx) = bezier_kappa((i-1)/(N_PTS-1), Pts);
            idx = idx + 1;
        end
    end
    kappas = gsort(kappas, "g", "i");  // sort ascending
    k95_idx = max(1, floor(length(kappas) * 0.95));
    kap95   = kappas(k95_idx);
    if kap95 > 1e-10 then
        scale = 0.15 * ext / kap95;
    else
        scale = 0.1;
    end

    // Tip positions for envelope line
    tip_x = [];
    tip_y = [];
    tip_z = [];

    comb_col = color("lightslategray");

    for i = 1:N_SPIKES
        t = (i-1) / (N_SPIKES-1);

        d1v  = bezier_d1(t, Pts);    // (1×3)
        d2v  = bezier_d2(t, Pts);    // (1×3)
        kap  = bezier_kappa(t, Pts);

        // U vector at t
        Uvec  = [t^4, t^3, t^2, t, 1];
        base  = Uvec * M * Pts;      // (1×3) point on curve

        // Binormal: B = d1 × d2
        bx = d1v(2)*d2v(3) - d1v(3)*d2v(2);
        by = d1v(3)*d2v(1) - d1v(1)*d2v(3);
        bz = d1v(1)*d2v(2) - d1v(2)*d2v(1);

        // Principal normal: N = B × d1
        nx = by*d1v(3) - bz*d1v(2);
        ny = bz*d1v(1) - bx*d1v(3);
        nz = bx*d1v(2) - by*d1v(1);
        ...
        // Tip = base + scale * kappa * N
        tip = base + scale * kap * [nx, ny, nz];

        // Draw spike: base -> tip
        draw_line3([base(1), tip(1)], [base(2), tip(2)], [base(3), tip(3)], ..
                   comb_col, 1, 1);
    ...
    // Envelope line connecting all tips
    if length(tip_x) > 1 then
        draw_line3(tip_x, tip_y, tip_z, comb_col, 1, 2);
    end
endfunction
...
function update_plot()
    global P u h_axes h_comb_check;
    ...
    // ── Curvature comb (if checkbox is on) ───────────────
    if is_handle_valid(h_comb_check) then
        if get(h_comb_check, "value") == 1 then
            draw_comb(P);
        end
    end
...
f = figure( ..
    "figure_name", "Quartic 3D Bezier + Curvature", ..
...
h_slider = uicontrol(f, "style","slider", ..
    ...
    "position",[10,85,185,22], ..
    "callback","sync_all()");

// ── Curvature comb checkbox ────────────────────────────────
h_comb_check = uicontrol(f, "style","checkbox", ..
    "string", "Curvature Comb", ..
    "value",  1, ..
    "position",[10,52,185,24], ..
    "callback","sync_all()");

// ── Reset button ──────────────────────────────────────────
uicontrol(f, "style","pushbutton","string","Reset", ..
    "position",[10,18,185,30], ..
    "callback","reset_data()");
