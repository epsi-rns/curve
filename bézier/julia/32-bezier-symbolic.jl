...
# Julia port of 32-bezier-scipy.py
# Uses GLMakie (interactive) + Symbolics.jl (curve evaluation)
# ============================================================
...
const CASE_DATA = Dict(
    1 => (
        ...
        u_dc        = 0.50,
        output      = "32-bezier-quadratic-2d.png",
    ),
    2 => (
        ...
        u_highlight = [0.00, 0.25, 0.50, 0.75, 1.00],
        u_dc        = 0.50,
        output      = "32-bezier-quartic-3d.png",
    ),
)
...
# Build B(n,i,u) symbolically once per degree, then substitute
# numeric u values — equivalent to scipy BPoly in the Python version.

@variables u_sym

function build_symbolic_poly(bz::BezierCurve)
    # Returns a Vector of symbolic expressions, one per spatial dim.
    # poly[d] = sum_i  B(n,i,u_sym) * P_i[d]
    n = bz.degree
    [
        sum(
            binomial(n, i-1) * u_sym^(i-1) * (1 - u_sym)^(n-(i-1)) * bz.points[i, d]
            for i in 1:bz.n
        )
        for d in 1:bz.dim
    ]
...
function bz_point(bz::BezierCurve, polys::Vector, u::Float64)
    [Float64(Symbolics.value(substitute(p, Dict(u_sym => u)))) for p in polys]
end

function bz_curve(bz::BezierCurve, polys::Vector, n_pts::Int=300)
    u_vals = LinRange(0.0, 1.0, n_pts)
    hcat([bz_point(bz, polys, u) for u in u_vals]...)'   # (n_pts × dim)
end

function bz_highlight(bz::BezierCurve, polys::Vector, u_list::Vector{Float64})
    hcat([bz_point(bz, polys, u) for u in u_list]...)'   # (m × dim)
end
...
function draw_and_save(
    ...
    # ── Highlighted points ────────────────────────────────────
    hpts = bz_highlight(bz, polys, u_highlight)
    if is_3d
